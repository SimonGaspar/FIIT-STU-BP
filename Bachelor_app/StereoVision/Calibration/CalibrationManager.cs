...
namespace Bachelor_app.StereoVision
{
    public class CalibrationManager
    {
        ...
        //buffers
        int buffer_length;
        int buffer_savepoint;
        MCvPoint3D32f[][] corners_object_Points;
        PointF[][] corners_points_Left;
        PointF[][] corners_points_Right;
        ...
        ExtrinsicCameraParameters EX_Param = new ExtrinsicCameraParameters();
        #endregion
        ...
        public void ProcessFrame()
        {
            ...
            {
                CameraHelper.GetStereoImage(_leftCamera, _rightCamera, ref frame_S1, ref frame_S2);

                _winForm.Video_Source1.Image = frame_S1.ImageBGR2Image();
                _winForm.Video_Source2.Image = frame_S1.ImageBGR2Image();

                switch (currentMode)
                {
                    case ECalibrationMode.SavingFrames:
                        Task.Run(async () => await SaveImageForCalibration(frame_S1, frame_S2));
                        break;
        ...
        {
            Matrix<float> MapX = null;
            Matrix<float> MapY = null;

            var size = _cameraManager.resolution.GetResolution();

            intrinsicCameraParameters.InitUndistortMap(size.Width, size.Height, out MapX, out MapY);
